...
IncStructure = namedtuple(
    "IncStructure",
    ["nvar", "ncon", "nnz", "nnode", "n_nl_node", "n_linear_node"],
)
MatchingElimResults = namedtuple("MatchingElimResults", ["n_matched"])
...
def get_structural_results(model, elim_callback):
    ...
    elim_res = MatchingElimResults(n_linear_match)
...
def main():
    ...
    nfe = 300
    models = [
        ("Distill", create_distill(horizon=horizon, nfe=nfe)),
        #("OPF-4917", create_opf()),
    ...
    for name, model in models:
        ...
        results = get_structural_results(model, matching_elim_callback)
        ...
        elimination_gap = (results.elim.n_matched - n_elim) / results.elim.n_matched
        ...
        print(f"Size of linear matching: {results.elim.n_matched}")
