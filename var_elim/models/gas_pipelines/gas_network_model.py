...
def make_model(
    ...
    m.fs.compressor = Compressor(m.fs.compressor_set, **compressor_config)

    # Connect compressors to pipelines

    m._compressor_to_pipeline_1 = network.Arc(
        ports=(m.fs.compressor[0].outlet_port, m.fs.pipeline[0].inlet_port)
    )

    m._compressor_to_pipeline_2 = network.Arc(
        ports=(m.fs.compressor[1].outlet_port, m.fs.pipeline[1].inlet_port)
    )

    m._compressor_to_pipeline_3 = network.Arc(
        ports=(m.fs.compressor[2].outlet_port, m.fs.pipeline[4].inlet_port)
    )
...
def make_dynamic_model(horizon=24.0, nxfe=4, ntfe=24, eta=0.7):
    ...
    m.fs.nodes[0].supplies[0].state[:].flow_mol.setub(1e10)
    ...
    tf = m.fs.time.last()
    [
        m.fs.pipeline[:].control_volume.flow_mass[tf, x].setlb(100000.0)
        for x in m.fs.pipeline[0].control_volume.length_domain
        if x != m.fs.pipeline[0].control_volume.length_domain.last()
    ]
    [
        m.fs.pipeline[:].control_volume.pressure[tf, x].setlb(20.0)
        for x in m.fs.pipeline[0].control_volume.length_domain
        if x != m.fs.pipeline[0].control_volume.length_domain.first()
    ]
