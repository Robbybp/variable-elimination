...
from var_elim.models.burgers.Burgers_model_pynumero import build_burgers_model
...
def main():
    m = build_burgers_model(nfe_x = 100, nfe_t = 100, end_t = 1)
    ipopt = pyo.SolverFactory("ipopt")
    ipopt.solve(m, tee= True)
    #Using AMPL heuristic for elimination
    t0 = time.time()
    var_list, con_list = identify_vars_for_elim_min_degree(m, eliminate_bounded_vars= True)
    t1 = time.time() - t0
