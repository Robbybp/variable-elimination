...
if __name__ == "__main__":
    ...
    if optimization:
        ...
        print(f"N. variables orig: {len(igraph.variables)}")
        print(f"N. constraints orig: {len(igraph.constraints)}")
        
        elim_vars, elim_cons = identify_vars_for_elim_ampl(m)
        elim_vars, elim_cons = define_elimination_order(elim_vars, elim_cons, igraph=igraph)
        eliminate_variables(m, elim_vars, elim_cons, igraph=igraph)

        reduced_igraph = IncidenceGraphInterface(m)
        print(f"N. variables reduced: {len(reduced_igraph.variables)}")
        print(f"N. constraints reduced: {len(reduced_igraph.constraints)}")
