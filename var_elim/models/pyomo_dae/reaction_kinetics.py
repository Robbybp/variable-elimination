...
# the results from the example using the coefficients that they give
#
def simple_optimization_model(nfe = 100):
    ...
    model = create_kinetic_model(rxns, 100 * 100)
    ...
    # initial conditions
    model.c[0, 'A'].fix(1)
    model.c[0, 'B'].fix(0.01)
    model.c[0, 'C'].fix(0.01)

    fdiff.apply_to(model, nfe=nfe)
...
if __name__ == "__main__":
    import sys
    simple_optimization_model(nfe = 2000)
