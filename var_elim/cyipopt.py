...
class TimedCyIpoptNLP(CyIpoptNLP):
    ...
    def objective(self, x):
        self._timer.start("objective")
        try:
            data = super().objective(x)
        finally:
            self._timer.stop("objective")
        return data

    def gradient(self, x):
        self._timer.start("objective-grad")
        try:
            data = super().gradient(x)
        finally:
            self._timer.stop("objective-grad")
        return data

    def constraints(self, x):
        self._timer.start("constraints")
        try:
            data = super().constraints(x)
        finally:
            self._timer.stop("constraints")
        return data

    def jacobian(self, x):
        self._timer.start("constraint-jac")
        try:
            data = super().jacobian(x)
        finally:
            self._timer.stop("constraint-jac")
        return data

    def hessian(self, x, y, obj_factor):
        self._timer.start("lagrangian-hess")
        try:
            data = super().hessian(x, y, obj_factor)
        finally:
            self._timer.stop("lagrangian-hess")
        return data
